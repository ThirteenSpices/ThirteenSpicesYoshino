...
function visi (tid , ttid)
    ...
    luajava.runUiThread(function()
        boxInnerCircles = boxInnerCircles 或 {}
...
function jianting3(func)
    ...
    for i = 1,#audiotype do
        ...
        elseif yinln[i].now < yinl[i].now 键，然后
            yinl[i].now = yinln[i].now
            if yltype == 0 then
                yltype = 1
                func()
...
function jianting(func)
    ...
    for i = 1,#audiotype do
        ...
        yinln[i].type = audiotype[i]
        yinln[i].now = audi:getStreamVolume(audiotype[i])
        if yinln[i].now >= yinl[i].max 键，然后
            audi:adjustStreamVolume(yinln[i].type,AudioManager.ADJUST_LOWER,0)
        end
        if yinln[i].now > yinl[i].now then
...
function changan.check(cklist)
    if #cklist==0 键，然后 return nil end
    local rest = {
        LinearLayout,
        layout_width = 'match_parent'，
        layout_height = "wrap_content"，
    ...
    ['功能配置']={
        ...
        changan.button("大厅防"， function()
            local function setvalue(address, flags, value)
                local tt = {[1] = {address = address, flags = flags, value = value}}
                gg.setValues(tt)
            end

            local gcloudRanges = gg.getRangesList('libgcloud.so')
            if not gcloudRanges or #gcloudRanges == 0 then
                gg.toast("未找到libgcloud.so模块")
                return
            end

            local so = gcloudRanges[1].start
            
            setvalue(so + 0x304A48, 4, -130022368)   -- 地址:0x7B22594A48
            setvalue(so + 0x304A98, 4, -113237020)   -- 地址:0x7B22594A98
            setvalue(so + 0x304AC0, 4, 1797342879)   -- 地址:0x7B22594AC0
            setvalue(so + 0x304AE8， 4, -763363197)   -- 地址:0x7B22594AE8
            setvalue(so + 0x304B4C， 4， 1384120448)   -- 地址:0x7B22594B4C
            setvalue(so + 0x304BD8， 4, -763363198)   -- 地址:0x7B22594BD8
            setvalue(so + 0x304C68， 4, -130021631)   -- 地址:0x7B22594C68
